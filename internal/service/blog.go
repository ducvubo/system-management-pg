...
type (
	IBlog interface {
		...
		FindBlogById(ctx context.Context, blId string, User *model.UserManagementProfileOutput) (out *model.BlogOutput, err error, statusCode int)
		GetAllBlog(ctx context.Context, Limit int32, Offset int32, isDeleted int32, User *model.UserManagementProfileOutput) (out response.ModelPagination[[]*model.BlogOutput], err error, statusCode int)
